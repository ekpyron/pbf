...
{
	const float h = std::sqrt(3.0f);
	pbf::NeighbourCellFinder::GridData gridData(
		glm::ivec3(-4, -4, -4),
		glm::ivec3(3, 3, 3),
		h
	);
    ...
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<float> distrib(-3.5f, 2.5f);
	...
	std::sort(particleInitData.begin(), particleInitData.end(),[&](auto a, auto b) {
		a.position /= gridData.cellSize;
		b.position /= gridData.cellSize;
		a.position -= glm::vec3(gridData.min);
		b.position -= glm::vec3(gridData.min);
    ...
    *gridDataBuffer.data() = gridData;
            ...
            pbf::descriptors::ComputePipeline{
                    ...
                    .shaderStage = pbf::descriptors::ShaderStage {
                            ...
                            .specialization = {
                            		...
                            		pbf::Specialization{.constantID = 1, .value = h}
                            }
                    },
    ...
    for(size_t i = 0; i < numValues; ++i)
        for(size_t j = 0; j < numValues; ++j)
            if (i != j)
                if (glm::distance(ptr[i].position, ptr[j].position) < h)
