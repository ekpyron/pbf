...
namespace pbf
{
SurfaceReconstruction::FrameData::FrameData(InitContext& _initContext, Renderer& _renderer,
                                            GlobalAppData& globalAppData, SurfaceReconstruction& _parent):
    depthInputImage(
    ...
    thicknessImage(
        ...
        // TODO: remove transfer src
        extent3D()
    )

{
    VulkanContext& context = _initContext.context;
    depthInputView = context.device().createImageViewUnique(vk::ImageViewCreateInfo{
        .flags = {},
        .image = depthInputImage.image(),
...

    {
            ...
            setLayouts.emplace_back(*layout);
        {
            ...
            int k = 0;
            depthBlurInputDescriptorSets.inputSampler = std::move(allocatedDescriptorSets.at(k++));
            depthBlurInputDescriptorSets.outputStorageImage = std::move(allocatedDescriptorSets.at(k++));
            depthBlurPingDescriptorSets.inputSampler = std::move(allocatedDescriptorSets.at(k++));
            depthBlurPingDescriptorSets.outputStorageImage = std::move(allocatedDescriptorSets.at(k++));
            depthBlurPongDescriptorSets.inputSampler = std::move(allocatedDescriptorSets.at(k++));
            depthBlurPongDescriptorSets.outputStorageImage = std::move(allocatedDescriptorSets.at(k++));
            reconstructNormalDescriptorSets.inputSampler = std::move(allocatedDescriptorSets.at(k++));
            reconstructNormalDescriptorSets.outputStorageImage = std::move(allocatedDescriptorSets.at(k++));
            // TODO: note: we're allocating one more unused descriptor set here currently. Do we?
        }
        ...
        vk::DescriptorImageInfo& inputImageInfo = imageInfos.emplace_back(vk::DescriptorImageInfo{
            .sampler = *_parent.depthAndColorSampler,
            .imageView = *depthInputView,
            .imageLayout = vk::ImageLayout::eGeneral // TODO: choose optimal layout
        });
        vk::DescriptorImageInfo& pingImageInfo = imageInfos.emplace_back(vk::DescriptorImageInfo{
            .sampler = *_parent.depthAndColorSampler,
            .imageView = *depthPingView,
            .imageLayout = vk::ImageLayout::eGeneral // TODO: choose optimal layout
        });
        vk::DescriptorImageInfo& pongImageInfo = imageInfos.emplace_back(vk::DescriptorImageInfo{
            .sampler = *_parent.depthAndColorSampler,
        ...
        // depth blur input descriptor writes
        {
            descriptorWrites.emplace_back(
                vk::WriteDescriptorSet{
                    .dstSet = *depthBlurInputDescriptorSets.inputSampler,
                    .dstBinding = 0,
                    .dstArrayElement = 0,
                    .descriptorCount = 1,
            ...
            descriptorWrites.emplace_back(
                vk::WriteDescriptorSet{
                    .dstSet = *depthBlurInputDescriptorSets.outputStorageImage,
                    .dstBinding = 0,
                    .dstArrayElement = 0,
                    .descriptorCount = 1,
                    .descriptorType = vk::DescriptorType::eStorageImage,
                    .pImageInfo = &pingImageInfo,
                    .pBufferInfo = nullptr,
                    .pTexelBufferView = nullptr
                }
            );
        ...
        // depth blur ping descriptor writes
        {
            descriptorWrites.emplace_back(
                vk::WriteDescriptorSet{
                    .dstSet = *depthBlurPingDescriptorSets.inputSampler,
                    .dstBinding = 0,
                    .dstArrayElement = 0,
                    .descriptorCount = 1,
                    .descriptorType = vk::DescriptorType::eCombinedImageSampler,
            ...
            descriptorWrites.emplace_back(
                vk::WriteDescriptorSet{
                    .dstSet = *depthBlurPingDescriptorSets.outputStorageImage,
                    .dstBinding = 0,
                    .dstArrayElement = 0,
                    .descriptorCount = 1,
                    .descriptorType = vk::DescriptorType::eStorageImage,
                    .pImageInfo = &pongImageInfo,
                    .pBufferInfo = nullptr,
                    .pTexelBufferView = nullptr
                }
            );
    ...
    frameSyncData(_renderer)
{
    ...
    depthAndColorSampler = context.device().createSamplerUnique(
        ...
        _reconstructNormalsPipeline = cache.fetch(
            descriptors::ComputePipeline{
                ...
                .shaderStage = descriptors::ShaderStage{
                    .module = cache.fetch(
                        descriptors::ShaderModule{
                            .source = descriptors::ShaderModule::File{
                                "shaders/surface/reconstruct_normals.comp.spv"
                            },
...
void SurfaceReconstruction::run(vk::CommandBuffer& _buf)
{
    ...
    _buf.pipelineBarrier(
        ...
        vk::PipelineStageFlagBits::eComputeShader, {}, {}, {}, {
            vk::ImageMemoryBarrier{
                ...
                .newLayout = vk::ImageLayout::eGeneral,
                .image = frameData.depthInputImage.image(),
                .subresourceRange = {
                    .aspectMask = vk::ImageAspectFlagBits::eDepth,
                    .baseMipLevel = 0,
                    .levelCount = 1,
    ...
    _buf.bindDescriptorSets(
        ...
        *_depthBlurPipeline->pipelineLayout,
        0,
        frameData.depthBlurInputDescriptorSets.all(),
        {}
    );
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        _buf.bindDescriptorSets(
            ...
            *_depthBlurPipeline->pipelineLayout,
            0,
            frameData.depthBlurPingDescriptorSets.all(),
            {}
        );
        _buf.dispatch(1024 / 256, 1024, 1);
